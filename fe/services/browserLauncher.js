    ...
    try {
      ...
      try {
        ...
        if (resp.status === 200) {
          ...
          console.log(`    [OK] IP: ${ip} | Timezone: ${timezone} | Location: ${location || 'Unknown'}`);
...
const buildRuyiConfig = (profile, proxyIP) => {
  ...
  console.log('[RUYI] Config preview (first 200 chars):', compactJson.substring(0, 200));
...
const createProxyAuthExtension = (host, port, user, pass) => {
  const pluginDir = path.join(os.tmpdir(), `proxy_auth_plugin_${Date.now()}_${Math.floor(Math.random() * 10000)}`);
...
const killBrowserByProfileId = async (profileId) => {
  ...
  try {
      ...
      try {
        ...
        exec(killCmd, (error, stdout, stderr) => {
          if (!error) {
            ...
            console.log(`[KILL] Windows kill error (may be normal): ${error.message}`);
          }
        });
      } catch (error) {
    ...
    console.error(`[KILL] Error killing browser for profile ${profileId}:`, error);
 ...
 */
const launchBrowser = async (profile, proxy, urlToOpen = 'https://whoer.net') => {
  console.log('\n========================================');
  ...
  try {
    ...
    if (!chromePath) {
      ...
      return { success: false, error: 'Không tìm thấy Chrome executable. Vui lòng cài đặt Google Chrome.' };
    }
    ...
    if (proxy) {
      ...
      if (!proxyConfig) {
        console.error('[ERROR] Proxy config không hợp lệ. Proxy object:', proxy);
      ...
      timezone = profile.timezone === 'auto' ? directInfo.timezone : profile.timezone;
    }
    ...
    try {
      ...
      console.log('[DEBUG] Screen:', `${ruyiConfigObj.screen_width}x${ruyiConfigObj.screen_height}`);
    ...
    // 3. Tạo user data dir
    const userDataDir = path.join(os.tmpdir(), `ruyi_live_${Date.now()}_${Math.floor(Math.random() * 10000)}`);
    ...
    // 7. Launch Chrome
    console.log(`[*] Mở Browser với Proxy: ${proxy ? parseProxyConfig(proxy)?.server : 'Direct'}...`);
    ...
    chromeProcess.on('exit', (code, signal) => {
      console.log(`[PROCESS] Chrome process exited for profile ${profile.id} with code ${code}, signal ${signal}`);
    ...
    return { success: false, error: error.message || 'Lỗi không xác định khi khởi động browser' };
  }
};
